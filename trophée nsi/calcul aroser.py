...
with open(r"D:\Concour_NSI\stockage(2).json", "r", encoding="utf-8") as f:                                            #fichier json contenant les plantes et leurs infos
    data = json.load(f)
...
def aroser(nom_courant,stade):

    plante = next(p for p in data["plantes"] if p["nom_courant"] == nom_courant)                                                        # Extraire toutes les valeurs directement depuis le JSON

    eau_deja_presente = plante["eau_deja_presente"]


    eau = plante["eau"]                                    # en cl
    dissipation_eau = plante["dissipation"]                #on recupere le pourcentage de dissipation de la plante par seconde
    ...
    + ((humidite_air_min + humidite_air_max) / 2 - humidite_air) / ((humidite_air_max - humidite_air_min) / 2)) - plante["eau_deja_presente"]

    plante["eau_deja_presente"] -= plante["dissipation"] * plante["eau_deja_presente"]                              #retire le pourcentage de dissipation de l'eau par seconde

    plante["eau_deja_presente"] += o_a_aroser

    return o_a_aroser

def notification(tittre,texte):
    ...                                                     #a faire avec tkinter
...
while True:
    print("tu dois aroser",int(aroser("Carotte",'maturité')),"cl (resultat en m²)")
    time.sleep(1)
