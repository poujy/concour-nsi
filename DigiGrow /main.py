...
# on stocke None et on affiche un message — l'arrosage sera désactivé.
...
# Compteur d'instances par espèce, pour numéroter les plantes (Rose1, Rose2, ...).
# Structure : { "Rose": 2, "Tulipe": 1, ... }
compteur_plantes = {}
...
def verif_climat_plante(nom_ville: str) -> list[str]:
    ...
    # On garde uniquement les plantes dont la saison courante est True
    # ET dont le climat est compatible avec celui de la ville sélectionnée.
    plantes_dispo = [
        plante["nom_courant"]
...
def correspond_climat(plante: dict, ville: dict) -> bool:
    ...
    # any() s'arrête au premier True : la plante est compatible si elle partage
    # au moins un type de climat avec la ville.
...
def aroser(nom_numerote: str, stade: str) -> tuple[float | None, float | None]:
    ...
    # rstrip("0123456789") retire les chiffres en fin de chaîne pour retrouver
    # le nom d'espèce de base. Ex : "Rose2" → "Rose".
