...
METEO_URL = ("http://api.weatherapi.com/v1/forecast.json?key=741a648e81bf44c28d7122536251410&q=Paris,France&days=14&aqi=no&alerts=no")
...
def conseil_pour_plante(nom: str) -> str:
    return CONSEILS.get(nom,f"Plantez votre {nom} dans un sol adapté à ses besoins en chaleur et en humidité. ""Consultez les indicateurs d'arrosage de l'application pour un suivi précis.")
...
def inscrire_plante_au_jardin(nom_plante: str, nom_numerote: str, stade: str) -> list:
    jardin["plantes"].append({"nom_numéroté":nom_numerote,"nom_courant":nom_plante,"stade":stade,"eau_deja_presente": 0.0,})
...
def calculer_cycle_arrosage(nom_numerote: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    eau_a_apporter = eau * stade_mul * (((ch_min  + ch_max)  / 2 - chaleur) / ((ch_max  - ch_min) / 2) + ((sol_min + sol_max) / 2 - soleil)   / ((sol_max - sol_min) / 2) + ((ha_min  + ha_max)  / 2 - humidite) / ((ha_max  - ha_min)  / 2))
    eau_a_apporter = max(eau_a_apporter, eau * 0.1)

    # dissipation en temp réele en fonction de la météo

    dissipation = dissipation_horaire * ((1 + 0.020 * (chaleur  - (ch_min  + ch_max)  / 2)) * (1 + 0.030 * (soleil   - (sol_min + sol_max) / 2)) * (1 - 0.015 * (humidite - (ha_min  + ha_max)  / 2)))
...
def naviguer_vers(nom: str) -> None:
    vider_contenu()
    pages = {"Plants":dessiner_jardin,"Handbook":dessiner_handbook,"Stats":dessiner_stats,"AjouterPlante": dessiner_formulaire_ajout}
    if nom in pages:
        pages[nom]()
...
def navbar() -> None:
    ...
    for icone, page in [("📘", "Handbook"), ("🌱", "Plants"), ("📊", "Stats")]:
        CTkButton(barre, text=icone, width=60, corner_radius=0,command=lambda p=page: naviguer_vers(p)).pack(side=LEFT, expand=True, fill=BOTH)
...
def dessiner_jardin() -> None:
    ...
    if not jardin["plantes"]:
        CTkLabel(content_frame,text="Aucune plante pour l'instant.\nAjoutez-en une ci-dessous !",font=("Arial", 13), text_color="#888888", justify="center").pack(pady=30)
    else:
        ...
        for p in jardin["plantes"]:
            ...
            if pct_eau > 0.6:
                ...
                clr_bar = "#F44336"
            carte = CTkFrame(scroll, corner_radius=12,fg_color=("#2b2b2b", "#1e1e1e"),border_width=1, border_color="#3a3a3a")
            ...
            CTkLabel(lbl_eau, text=f"{eau_ml / 1000:.2f} L", font=("Arial", 11), text_color="#888888").pack(side=RIGHT)     # Affichage en cL 
            barre = CTkProgressBar(carte, progress_color=clr_bar, height=8, corner_radius=4)
            barre.pack(fill=X, padx=12, pady=(2, 4))
            barre.set(pct_eau)
            CTkLabel(carte, text=f"⏱ Prochain arrosage : {prochain}",font=("Arial", 11), text_color="#aaaaaa", anchor="w").pack(anchor="w", padx=12, pady=(0, 6))
            CTkButton(carte, text="💧 Je l'ai arrosé",width=180, height=30, corner_radius=8,fg_color="#1a6b3a", hover_color="#145c30",font=("Arial", 12),command=lambda n=nom_num: recharger_eau_manuellement(n)).pack(pady=(0, 10))
    CTkButton(content_frame, text="➕  Ajouter une plante",width=240, height=40, corner_radius=20,command=lambda: naviguer_vers("AjouterPlante")).pack(pady=10)

#  la page d'ajout de plantes   
                                             
def dessiner_formulaire_ajout() -> None:
    CTkButton(content_frame, text="← Retour", width=100,anchor="w", fg_color="transparent",command=lambda: naviguer_vers("Plants")).pack(anchor="w", padx=10, pady=(10, 0))
    ...
    combo_stade = CTkComboBox(content_frame,values=["tout juste planté", "maturité", "floraison"],state="readonly", width=220)
    ...
    def confirmer_ajout():
        ...
        if nom not in ("Choisissez une plante...", "Aucune plante disponible"):
            ...
            naviguer_vers("Plants")       
    CTkButton(content_frame, text="Ajouter", width=200, height=40, corner_radius=20,command=confirmer_ajout).pack(pady=25)
...
def dessiner_handbook() -> None:
    ...
    CTkLabel(content_frame, text="Sélectionnez une plante pour ses conseils",font=("Arial", 12), text_color="#888888").pack(pady=(0, 8))
    scroll = CTkScrollableFrame(content_frame, fg_color="transparent")
    scroll.pack(fill=BOTH, expand=True, padx=10, pady=5)
    ...
    for i, nom in enumerate(toutes):
        ...
        CTkButton(ligne, text=nom, width=155, height=40, corner_radius=10,command=lambda n=nom: ouvrir_fiche_plante(n)).grid(row=0, column=col, padx=4)

def ouvrir_fiche_plante(nom_plante: str) -> None:
    vider_contenu()
    CTkButton(content_frame, text="← Retour", width=100,anchor="w", fg_color="transparent",command=lambda: naviguer_vers("Handbook")).pack(anchor="w", padx=10, pady=(10, 0))
    ...
    if ref:
        ...
        for label, valeur in [("🌡  Température",f"{ref['besoin_chaleur_min']}-{ref['besoin_chaleur_max']} °C"),("☀️  Soleil (UV)",f"{ref['besoin_soleil_min']}-{ref['besoin_soleil_max']}"),("💧  Eau / cycle",f"{ref['eau'] / 1000:.2f} L"),("💨  Humidité air",f"{ref['humidite_air_min']}-{ref['humidite_air_max']} %")]:
    ...
    CTkLabel(conseil_frame, text="💡 Conseils", font=("Arial", 13, "bold"), anchor="w").pack(anchor="w", padx=12, pady=(10, 4))
    CTkLabel(conseil_frame, text=conseil_pour_plante(nom_plante),font=("Arial", 12), wraplength=300, justify="left", anchor="w").pack(anchor="w", padx=12, pady=(0, 12))
...
def dessiner_stats() -> None:
    ...
    def carte_stat(icone: str, titre: str, valeur: str) -> None:
        carte =CTkFrame(content_frame, corner_radius=12,fg_color=("#2b2b2b", "#1e1e1e"),border_width=1, border_color="#3a3a3a")
        carte.pack(fill=X, padx=15, pady=5)
        CTkLabel(carte, text=f"{icone}  {titre}",font=("Arial", 12),text_color="#888888", anchor="w").pack(anchor="w", padx=12, pady=(8, 2))
